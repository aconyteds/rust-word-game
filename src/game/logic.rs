...
pub fn create_suggestion(guesses: &Vec<Guess>) -> String {
    ...
    for guess in guesses {
        ...
        for f in feedback {
            ...
            } else if f.in_correct_location {
                filtered_words_clone
                    .retain(|w| w.chars().nth(f.letter as usize - 97).unwrap() == f.letter);
            } else {
                filtered_words_clone.retain(|w| !w.contains(f.letter));
            }
        }
